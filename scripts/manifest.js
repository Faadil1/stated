...
const { canonicalize: jcsCanonical } = require("json-canonicalize");

const DECLARATION_SCHEMA_VERSION = "stated/declaration/v1";
const EVIDENCE_SCHEMA_VERSION = "stated/evidence/v1";

/**
 * RFC 8785 JSON Canonicalization Scheme
 * Uses the maintained json-canonicalize library for RFC 8785 compliance
 * Handles: number precision, Unicode, escaped characters, key ordering per spec
 * @param {object} obj - Object to canonicalize
 * @returns {string} - Canonical JSON string per RFC 8785
 */
...
/**
 * Compute deterministic hash of a manifest
 * Uses: keccak256(UTF8(RFC8785(JSON)))
 * @param {object} manifest - Manifest object
 * @returns {string} - Keccak256 hash (0x-prefixed hex)
 */
...
/**
 * Validate a declaration manifest
 * @param {object} manifest - Declaration manifest
...
function validateDeclaration(manifest) {
  ...
  if (manifest.schema !== DECLARATION_SCHEMA_VERSION) {
    throw new Error(
      `Invalid schema. Expected "${DECLARATION_SCHEMA_VERSION}", got "${manifest.schema}"`
    );
  }
  ...
  if (
    !manifest.project.promise ||
    typeof manifest.project.promise !== "string"
  ) {
 ...
 * @param {object} declaration - Declaration manifest for validation
 * @throws {Error} - If manifest is invalid
 */
function validateEvidence(manifest, declaration) {
  if (!manifest || typeof manifest !== "object") {
    throw new Error("Manifest must be an object");
  }

  if (manifest.schema !== EVIDENCE_SCHEMA_VERSION) {
    throw new Error(
      `Invalid schema. Expected "${EVIDENCE_SCHEMA_VERSION}", got "${manifest.schema}"`
    );
  }
  ...
  // Build set of valid condition IDs from declaration
  ...
  for (const item of manifest.evidence) {
    ...
    for (const condId of item.conditionIds) {
      if (!validConditionIds.has(condId)) {
        throw new Error(
          `Unknown condition ID in evidence: ${condId}`
        );
      }
    }
    ...
    if (item.type && typeof item.type !== "string") {
      throw new Error("evidence.type must be a string if present");
    }
  }
